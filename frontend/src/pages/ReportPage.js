...
const loadReport = useCallback(async () => {
  try {
    ...
    // ── DEBUG: remove this console.log after confirming the fix works ──
    console.log('t3 raw:', JSON.stringify(res.data.report.table3_review_hazards));
...
useEffect(() => { loadReport(); }, [loadReport]);
  ...
  const ynk = (val) => {
    ...
    if (typeof val === 'string') {
      ...
      if (l === 'yes' || l === '1') return 'Yes';
      if (l === 'no'  || l === '0') return 'No';
    }
    // Buffer from MySQL
  ...
  /*
   * checked() — converts any MySQL boolean representation to ☑ or ☐.
   *
   * MySQL TINYINT(1) columns can arrive as:
   *   • true / false          (when typeCast is enabled in pool)
   *   • 1 / 0                 (number)
   *   • "1" / "0"             (string)
   *   • "yes" / "no"          (string — if your query casts them)
   *   • { type:'Buffer', data:[1] }  (Buffer — when typeCast is OFF)
   *
   * The previous bug: any non-empty string (including "no") is truthy
   * in JS, so checked("no") returned ☑. Fixed below.
   *
   * This function also accepts a JS boolean expression directly,
   * e.g. checked(t3.exterior_review === 'partial') — those always
   * produce a real true/false so they're handled by the last line.
   */
  const checked = (val) => {
    // null / undefined / empty → unchecked
    if (val === null || val === undefined || val === '') return '☐';

    // MySQL Buffer { type: 'Buffer', data: [0|1] }
    ...
    if (typeof val === 'string') {
      ...
      if (lower === 'yes' || lower === '1' || lower === 'true')  return '☑';
      if (lower === 'no'  || lower === '0' || lower === 'false') return '☐';
      // Any other non-empty string — treat as unchecked to be safe
      return '☐';
    }

    // Number: 1 → ☑, anything else (0, NaN…) → ☐
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '.58rem', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '.12em' }}>Assessment #{id}</span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ fontSize: '.9rem', fontWeight: '800', color: '#0f172a', letterSpacing: '-.02em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t1.building_name}</span>
              <span style={{ fontSize: '.72rem', color: '#94a3b8', fontWeight: '500', whiteSpace: 'nowrap' }}>— {new Date(t1.assessment_date).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span>
            </div>
          </div>
      ...
      <div className="rp-wrap" style={{ padding: '28px', background: '#f8fafc', minHeight: '100vh' }}>
        ...
        <div className="rp-banner" style={{ maxWidth: '1100px', margin: '0 auto 20px', background: 'linear-gradient(135deg,#0f172a 0%,#1e1b4b 100%)', borderRadius: '20px', padding: '20px 28px', display: 'flex', alignItems: 'center', gap: '20px', flexWrap: 'wrap', boxShadow: '0 12px 32px -8px rgba(15,23,42,.35)', overflow: 'hidden', position: 'relative' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, position: 'relative', zIndex: 1 }}>
            <div style={{ fontSize: '.58rem', fontWeight: '800', color: 'rgba(255,255,255,.2)', textTransform: 'uppercase', letterSpacing: '.14em', marginBottom: '4px' }}>RVS Level 1 Screening · FEMA P-154 · HIGH Seismicity</div>
            <div style={{ fontSize: '1.05rem', fontWeight: '800', color: 'white', letterSpacing: '-.02em' }}>{t1.building_name}</div>
            <div style={{ fontSize: '.76rem', color: 'rgba(255,255,255,.35)', fontWeight: '500', marginTop: '3px' }}>{t1.address}</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexShrink: 0, position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ padding: '10px 20px', borderRadius: '12px', background: isPass ? 'rgba(16,185,129,.18)' : 'rgba(239,68,68,.18)', border: `1px solid ${isPass ? 'rgba(16,185,129,.35)' : 'rgba(239,68,68,.35)'}`, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '.58rem', fontWeight: '700', color: 'rgba(255,255,255,.22)', textTransform: 'uppercase', letterSpacing: '.08em', marginTop: '1px' }}>{isPass ? 'No L2 Required' : 'L2 Required'}</div>
            </div>
          </div>
        </div>
          ...
          <div ref={printRef} className="rvs-print-form" style={{ background: 'white', padding: '10px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: '0', border: '2px solid #000' }}>
              ...
              <div style={{ borderRight: '1px solid #000' }}>
                <div className="rvs-photo-box" style={{ borderBottom: '1px solid #000' }}>
                  {t2.length > 0 ? (
                    <img src={t2[0].url} alt="Building" style={{ width: '100%', height: '140px', objectFit: 'cover' }} onError={(e) => { e.target.style.display = 'none'; }} />
                  ) : (
                    <div style={{ height: '140px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#999', fontSize: '11px', textAlign: 'center' }}>PHOTOGRAPH</div>
                  )}
                </div>
                <div className="rvs-sketch-box">
                  <div style={{ textAlign: 'center', fontSize: '10px', color: '#999', padding: '60px 0' }}>SKETCH</div>
                </div>
                <div style={{ padding: '4px', fontSize: '9px', borderTop: '1px solid #ccc' }}>☐ Additional sketches or comments on separate page</div>
              </div>
            ...
            <div style={{ border: '2px solid #000', borderTop: 'none' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', borderBottom: '1px solid #000' }}>
                ...
                <tbody>
                  {[
                    { label: 'Basic Score',                        values: ['', 3.6, 3.2, 2.9, 2.1, 2.0, 2.6, 2.0, 1.7, 1.5, 2.0, 1.2, 1.6, 1.4, 1.7, 1.7, 1.0, 1.5] },
                    { label: 'Severe Vertical Irregularity, V₁s',  values: ['', -1.2, -1.2, -1.2, -1.0, -1.0, -1.1, -1.0, -0.8, -0.9, -1.0, -0.7, -1.0, -0.9, -0.9, -0.9, -0.7, 'NA'] },
                    { label: 'Moderate Vertical Irregularity, V₁t', values: ['', -0.7, -0.7, -0.7, -0.6, -0.6, -0.7, -0.6, -0.5, -0.5, -0.6, 'NA', -0.6, -0.5, -0.5, -0.5, -0.4, 'NA'] },
                    { label: 'Plan Irregularity, P₁t',              values: ['', -1.1, -1.0, -1.0, -0.8, -0.7, -0.9, -0.7, -0.6, -0.6, -0.8, -0.5, -0.7, -0.6, -0.7, -0.7, -0.4, 'NA'] },
                    { label: 'Pre-Code',                            values: ['', -1.1, -1.0, -0.9, -0.6, -0.6, -0.8, -0.6, -0.2, -0.4, -0.7, -0.1, -0.5, -0.3, -0.5, -0.5, 0, -0.1] },
                    { label: 'Post-Benchmark',                      values: ['', 1.6, 1.9, 2.2, 1.4, 1.4, 1.1, 1.9, 'NA', 1.9, 2.1, 'NA', 2.0, 2.4, 2.1, 2.1, 'NA', 1.2] },
                    { label: 'Soil Type A or B',                    values: ['', 0.1, 0.3, 0.5, 0.4, 0.6, 0.1, 0.6, 0.5, 0.4, 0.5, 0.3, 0.6, 0.4, 0.5, 0.5, 0.3, 0.3] },
                    { label: 'Soil Type E (1-3 stories)',            values: ['', 0.2, 0.2, 0.1, -0.2, -0.4, 0.2, -0.1, -0.4, 0.0, 0.0, -0.2, -0.3, -0.1, -0.1, -0.1, -0.2, -0.4] },
                    { label: 'Soil Type E (> 3 stories)',            values: ['', -0.3, -0.6, -0.9, -0.6, -0.6, 'NA', -0.6, -0.4, -0.5, -0.7, -0.3, 'NA', -0.4, -0.5, -0.6, -0.2, 'NA'] },
                    { label: 'Minimum Score, Smin',                  values: ['', 1.1, 0.9, 0.7, 0.5, 0.5, 0.6, 0.5, 0.5, 0.3, 0.3, 0.3, 0.2, 0.2, 0.3, 0.3, 0.2, 1.0] },
                  ].map((row, ri) => (
                    <tr key={ri} style={{ background: ri % 2 === 0 ? '#fff' : '#f8f8f8' }}>
                      ...
                      {row.values.map((v, vi) => {
                        ...
                        const isDNK = vi === 0;
                        return (
                          <td key={vi} style={{ textAlign: 'center', fontSize: '8px', padding: '1px 2px', background: isSelected ? '#fff3cd' : 'transparent', fontWeight: isSelected ? 700 : 400, border: isSelected ? '1.5px solid #000' : '1px solid #ddd', color: isDNK ? 'transparent' : 'inherit' }}>
                            {isDNK ? '' : v}
                          </td>
                        );
                      })}
              ...
              <div style={{ padding: '6px 8px', fontWeight: 700, fontSize: '11px', background: '#f0f0f0' }}>
                ...
                &nbsp;&nbsp;
                <span style={{ color: t1.result === 'PASS' ? '#155724' : '#721c24' }}>→ {t1.result}</span>
              </div>
            </div>
            ...
            <div className="non-fema-section" style={{ marginTop: '30px' }}>
              ...
              <div style={{ marginBottom: '20px' }}>
                <div style={{ background: '#1e3a5f', color: '#fff', padding: '6px 10px', fontWeight: 700, fontSize: '11px' }}>TABLE 1: BUILDING INFORMATION AND SEISMIC SCORE SUMMARY</div>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px', border: '1px solid #ccc' }}>
                  <tbody>
                    ...
                    <tr style={{ background: t1.result === 'PASS' ? '#d4edda' : '#f8d7da' }}>
                      ...
                      <td style={{ padding: '5px 8px', fontWeight: 700, fontSize: '16px', color: t1.result === 'PASS' ? '#155724' : '#721c24' }}>{t1.result} {t1.result === 'PASS' ? '✓' : '✗'}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div style={{ marginBottom: '20px' }}>
                <div style={{ background: '#1e3a5f', color: '#fff', padding: '6px 10px', fontWeight: 700, fontSize: '11px' }}>TABLE 3: EXTENT OF REVIEW AND OTHER HAZARDS</div>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px', border: '1px solid #ccc' }}>
                  <tbody>
                    <tr>
              ...
              <div>
                <div style={{ background: '#1e3a5f', color: '#fff', padding: '6px 10px', fontWeight: 700, fontSize: '11px' }}>TABLE 4: ACTION REQUIRED</div>
